    ...
    <div className="animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-12 mb-20 border-b border-black/5 pb-16">
        <div className="flex flex-wrap gap-3 justify-center">
          {['All', ...Object.values(Category)].map(cat => (
            <button
              ...
              className={`px-6 py-3 text-[14px] font-bold rounded-full transition-all ${
        ...
        <button 
          ...
          className={`px-8 py-3 rounded-full font-extrabold text-[14px] uppercase tracking-widest transition-all border-2 ${
            isUrgent 
      ...
      {assistantMessage && (
        <div className="bg-[#f5f5f7] rounded-[48px] p-16 mb-32 flex items-start gap-12 animate-in slide-in-from-top-6 duration-700 apple-shadow">
          <div className="text-5xl">⚡️</div>
          <div className="flex-grow">
            <p className="text-[11px] font-bold text-[#86868b] mb-3 uppercase tracking-[0.4em]">Expert Retrieval Agent</p>
            <p className="text-black text-3xl font-extrabold leading-tight tracking-tight">"{assistantMessage}"</p>
          </div>
          <button 
            onClick={() => { setAssistantMessage(null); setFilteredVendors(vendors); }}
            className="p-4 bg-white rounded-full hover:bg-black hover:text-white transition-all shadow-sm"
          >
