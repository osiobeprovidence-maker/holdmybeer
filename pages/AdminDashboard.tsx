...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ users, serviceRequests, onExit, onUpdateUser }) => {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-[80vh] flex items-center justify-center p-4 md:p-6 animate-in fade-in duration-1000">
        <div className="w-full max-w-md bg-white border border-black/5 rounded-[40px] md:rounded-[60px] p-8 md:p-16 apple-shadow-lg text-center">
          <div className="w-16 h-16 md:w-20 md:h-20 bg-black flex items-center justify-center rounded-[20px] md:rounded-[28px] mx-auto mb-8 md:mb-10">
            <span className="text-3xl md:text-4xl">🐺</span>
          </div>
          <h1 className="text-3xl md:text-4xl font-black uppercase tracking-tighter mb-4">Admin Protocol</h1>
          <p className="text-[9px] md:text-[10px] font-bold text-[#86868b] uppercase tracking-[0.4em] mb-8 md:mb-12">Authorized Infrastructure Access Only</p>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <input
              ...
              className="w-full bg-[#f5f5f7] border-none rounded-[24px] md:rounded-[32px] p-5 md:p-6 text-center text-base md:text-lg font-black outline-none tracking-[0.2em] focus:ring-4 focus:ring-black/5 transition-all"
            />
            {error && <p className="text-red-500 text-[9px] md:text-[10px] font-bold uppercase tracking-widest">{error}</p>}
            <button type="submit" className="w-full btn-apple py-5 md:py-6 text-[12px] md:text-[14px] uppercase tracking-[0.3em] shadow-xl">Authenticate</button>
          </form>
          <button onClick={onExit} className="mt-8 md:mt-12 text-[#86868b] text-[9px] md:text-[10px] font-bold uppercase tracking-widest hover:text-black transition-colors">Return to Surface</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="py-8 md:py-12 animate-in fade-in duration-700 max-w-7xl mx-auto px-4 md:px-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-10 mb-12 md:mb-16 border-b border-black/5 pb-10 md:pb-12 text-center md:text-left">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8 mb-16 md:mb-24">
        <div className="bg-[#f5f5f7] p-6 md:p-10 rounded-[32px] md:rounded-[48px] apple-shadow">
          <p className="text-[9px] md:text-[10px] font-bold text-[#86868b] uppercase tracking-widest mb-3 md:mb-4">Total Handshakes</p>
          <p className="text-2xl md:text-5xl font-black">{totalHandshakes}</p>
        </div>
        <div className="bg-[#f5f5f7] p-6 md:p-10 rounded-[32px] md:rounded-[48px] apple-shadow">
          <p className="text-[9px] md:text-[10px] font-bold text-[#86868b] uppercase tracking-widest mb-3 md:mb-4">Throughput (NGN)</p>
          <p className="text-2xl md:text-5xl font-black">₦{totalRevenue.toLocaleString()}</p>
        </div>
        <div className="bg-[#f5f5f7] p-6 md:p-10 rounded-[32px] md:rounded-[48px] apple-shadow">
          <p className="text-[9px] md:text-[10px] font-bold text-[#86868b] uppercase tracking-widest mb-3 md:mb-4">Expert Nodes</p>
          <p className="text-2xl md:text-5xl font-black">{totalExperts}</p>
        </div>
        <div className="bg-[#f5f5f7] p-6 md:p-10 rounded-[32px] md:rounded-[48px] apple-shadow">
          <p className="text-[9px] md:text-[10px] font-bold text-[#86868b] uppercase tracking-widest mb-3 md:mb-4">Verified Nodes</p>
          <p className="text-2xl md:text-5xl font-black">{verifiedExperts}</p>
        </div>
      </div>
