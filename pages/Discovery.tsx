  ...
  return (
    <div className="py-12 animate-in fade-in duration-700">
      <div className="mb-20 flex flex-col md:flex-row md:items-end justify-between gap-10">
        <div>
          <h1 className="text-6xl md:text-8xl font-extrabold mb-4 tracking-tighter text-black uppercase">Discovered</h1>
          <p className="text-[#86868b] font-bold uppercase text-[11px] tracking-[0.4em] opacity-50">Verified professionals ready to execute.</p>
        </div>
        
        <button 
          onClick={() => setIsUrgent(!isUrgent)}
          className={`px-10 py-4 text-[12px] font-black uppercase tracking-[0.2em] transition-all border-2 rounded-full ${
            isUrgent 
      ...
      {/* Search and Filters */}
      <div className="bg-white rounded-[48px] p-8 md:p-12 border border-black/5 apple-shadow-lg mb-24">
        <div className="flex flex-col md:flex-row gap-6 items-center">
          <div className="relative flex-grow w-full">
            ...
            />
            <button className="absolute right-3 top-1/2 -translate-y-1/2 btn-apple px-8 py-3 text-[11px] uppercase tracking-widest hover:scale-105">Find</button>
          </div>
          
          <button 
            onClick={() => setShowFilters(!showFilters)}
            className="w-full md:w-auto px-10 py-5 bg-white border border-black/10 rounded-[28px] text-[11px] font-black uppercase tracking-[0.2em] hover:bg-black hover:text-white transition-all"
          >
            ...
            return (
              <section key={cat} className="animate-in fade-in duration-700">
                <div className="flex justify-between items-end mb-12">
                  <h2 className="text-4xl font-extrabold uppercase tracking-tighter text-black">{cat} Experts</h2>
                  <button 
                    onClick={() => {
                      setCategoryFilter(cat);
                      setShowFilters(true);
                    }} 
                    className="text-black text-[11px] font-black uppercase tracking-[0.2em] border-b-2 border-black pb-1 hover:opacity-50 transition-opacity"
                  >
                    View All {cat}
                  </button>
                </div>
