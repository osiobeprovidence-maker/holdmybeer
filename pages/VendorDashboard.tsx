...
const VendorDashboard: React.FC<DashboardProps> = ({ user, onUpdateUser, unlockedVendors, serviceRequests, allUsers }) => {
  ...
  return (
    <div className="py-12 animate-in fade-in duration-700 max-w-5xl mx-auto px-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-10 mb-20 border-b border-black/5 pb-16">
        <div className="flex items-center gap-8">
           <img 
              src={user.avatar || `https://ui-avatars.com/api/?name=${user.name}`} 
              className="w-24 h-24 rounded-[40px] object-cover border-4 border-white apple-shadow" 
           />
           <div>
              <h1 className="text-4xl font-black tracking-tighter uppercase">{user.businessName || user.name}</h1>
        ...
        <button 
          ...
          className={`px-12 py-5 rounded-full font-black text-[12px] uppercase tracking-[0.2em] transition-all ${user.isCreator ? 'bg-black text-white' : 'bg-[#f5f5f7] text-black hover:bg-black hover:text-white'}`}
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
        <div className="lg:col-span-2">
          ...
          {activeTab === 'spot' && (
            <div className="bg-white border border-black/5 p-12 md:p-16 rounded-[60px] apple-shadow">
          ...
          {activeTab === 'verification' && (
            <div className="bg-[#f5f5f7] p-12 md:p-20 rounded-[64px] min-h-[500px] flex flex-col justify-center">
               {kycStep === 'phone' && (
        ...
        {/* Status Sidebar */}
        <div className="space-y-10">
           <div className="bg-white border border-black/5 p-12 rounded-[56px] apple-shadow sticky top-32">
