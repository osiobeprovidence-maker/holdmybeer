...
const MyConnections: React.FC<MyConnectionsProps> = ({
  ...
  return (
    <div className="py-12 animate-in fade-in duration-700">
      <div className="mb-12 md:mb-24 flex flex-col md:flex-row justify-between items-start md:items-end gap-6 md:gap-10 px-4 md:px-0">
        <div className="w-full">
          <p className="text-[10px] md:text-[12px] font-bold text-[#86868b] uppercase tracking-[0.4em] mb-2 md:mb-4">Historical Access</p>
          <h1 className="text-5xl md:text-8xl font-black mb-4 md:mb-6 tracking-tighter text-black uppercase leading-none">History</h1>
          <div className="flex flex-col md:flex-row md:items-center gap-4">
            <p className="text-[#86868b] font-bold uppercase text-[10px] md:text-[11px] tracking-widest opacity-50">Saved connection instances available on this device.</p>
            <div className="bg-black/5 px-3 py-1.5 rounded-full flex items-center gap-2 border border-black/5 self-start">
              <div className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse" />
              <span className="text-[8px] md:text-[9px] font-black uppercase tracking-tighter font-mono">NODE ID: {protocolId}</span>
            </div>
          </div>
        </div>
        {!currentUser && (
      ...
      <div className="grid grid-cols-1 gap-10">
        {connectedVendors.map(vendor => {
          ...
          return (
            <div key={vendor.id} className="apple-card p-6 md:p-12 apple-shadow flex flex-col md:flex-row gap-8 md:gap-12 items-center mx-4 md:mx-0">
              <div className="w-24 h-24 md:w-48 md:h-48 flex-shrink-0">
                <img src={vendor.avatar} className="w-full h-full rounded-[30px] md:rounded-[40px] object-cover apple-shadow" />
              </div>

              <div className="flex-grow text-center md:text-left w-full">
                <p className="text-[9px] md:text-[10px] font-bold text-black uppercase tracking-widest mb-1.5 opacity-30">{vendor.category}</p>
                <h3 className="text-2xl md:text-4xl font-extrabold text-black tracking-tight mb-6 md:mb-8 uppercase">{vendor.businessName}</h3>
        ...
        {connectedVendors.length === 0 && (
          <div className="bg-[#f5f5f7] py-24 md:py-40 rounded-[32px] md:rounded-[60px] text-center px-6 md:px-12 border border-black/5 mx-4 md:mx-0">
            <h2 className="text-4xl md:text-5xl font-extrabold text-black mb-4 md:mb-6 tracking-tight opacity-10 uppercase">Null History</h2>
            <p className="text-[#86868b] font-bold uppercase tracking-widest text-[10px] md:text-[11px] max-w-md mx-auto mb-10 md:mb-12 opacity-50">No instances discovered yet. Initiate a handshake on the home page to start.</p>
            <button onClick={() => window.location.hash = 'home'} className="btn-apple w-full md:w-auto px-10 md:px-16 py-5 md:py-6 font-black uppercase tracking-widest">Discovered Experts</button>
          </div>
        )}
      </div>
    </div>
